...
/**
 * @version 2014/10/02 15:13:34
 */
// @JavaAPIProvider(FilteredList.class)
public class FilteredListJS<E> extends TransformationList<E, E> {
    ...
    /**
     * Constructs a new FilteredList wrapper around the source list. The provided predicate will
     * match the elements in the source list that will be visible.
     * 
     * @param source the source list
     * @param predicate the predicate to match the elements. Cannot be null.
     */
    public FilteredListJS(@NamedArg("source") ObservableList<E> source, @NamedArg("predicate") Predicate<? super E> predicate) {
        super(source);
        if (predicate == null) {
    ...
    /**
     * Constructs a new FilteredList wrapper around the source list. This list has an "always true"
     * predicate, containing all the elements of the source list.
     * <p>
     * This constructor might be useful if you want to bind {@link #predicateProperty()} of this
     * list.
     * 
     * @param source the source list
     */
    public FilteredListJS(@NamedArg("source") ObservableList<E> source) {
        this(source, ALWAYS_TRUE);
    }

    /**
     * The predicate that will match the elements that will be in this FilteredList. Elements not
     * matching the predicate will be filtered-out.
     */
    ...
    private void updateFilter(int sourceFrom, int sourceTo) {
        ...
        if (sourceFrom == sourceTo - 1) {
            ...
            for (; i < filterTo; ++i) {
                ...
                // Then we look at the elements that are between the current element in filtered[]
                // array
                // and it's successor
