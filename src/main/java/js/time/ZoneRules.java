...
class ZoneRules {
     ...
     * <p>
     * The mapping from a local date-time to an offset is not straightforward. There are three
     * cases:
     * <ul>
     * <li>Normal, with one valid offset. For the vast majority of the year, the normal case
     * applies, where there is a single valid offset for the local date-time.</li>
     * <li>Gap, with zero valid offsets. This is when clocks jump forward typically due to the
     * spring daylight savings change from "winter" to "summer". In a gap there are local date-time
     * values with no valid offset.</li>
     ...
     * date-time values with two valid offsets.</li>
     * </ul>
     * Thus, for any given local date-time there can be zero, one or two valid offsets. This method
     * returns the single offset in the Normal case, and in the Gap or Overlap case it returns the
     ...
     * </ul>
     * Thus, for any given local date-time there can be zero, one or two valid offsets. This method
     * returns that list of valid offsets, which is a list of size 0, 1 or 2. In the case where
     * there are two offsets, the earlier offset is returned at index 0 and the later offset at
     * index 1.
     * <p>
     * There are various ways to handle the conversion from a {@code LocalDateTime}. One technique,
     * using this method, would be: <pre>
     ...
     * history of time-zones and thus has no special handling. However, if it were to happen, then
     * the list would return more than 2 entries.
     *
     * @param localDateTime the local date-time to query for valid offsets, not null, but null may
    ...
    private Object getOffsetInfo(LocalDateTime dt) {
        ...
        if (index < 0) {
            ...
            if (index < savingsLocalTransitions.length - 1 && savingsLocalTransitions[index].equals(savingsLocalTransitions[index + 1])) {
     ...
     * <p>
     * This provides access to historic information on how the standard offset has changed over
     * time. The standard offset is the offset before any daylight saving time is applied. This is
     * typically the offset applicable during winter.
     *
     ...
     * <p>
     * This provides access to historic information on how the amount of daylight savings has
     ...
     * {@link #getOffset(java.time.Instant) actual} and {@link #getStandardOffset(java.time.Instant)
     * standard} offsets.
     *
     * @param instant the instant to find the daylight savings for, not null, but null may be
     *            ignored if the rules have a single offset for all instants
     ...
     * <p>
     * This checks if the standard offset and the actual offset are the same for the specified
     * instant. If they are not, it is assumed that daylight savings is in operation.
     * <p>
     * This default implementation compares the {@link #getOffset(java.time.Instant) actual} and
     ...
     * <p>
     * To be valid, the local date-time must not be in a gap and the offset must match one of the
     * valid offsets.
     * <p>
     ...
     *
     * @param instant the instant to get the next transition after, not null, but null may be
     *            ignored if the rules have a single offset for all instants
     ...
     * {@link #getTransitionRules()}. This method returns those transitions that have been fully
     * defined. These are typically historical, but may be in the future.
     * <p>
     ...
     * <p>
     * The complete set of transitions for this rules instance is defined by this method and
     * {@link #getTransitions()}. This method returns instances of {@link ZoneOffsetTransitionRule}
     * that define an algorithm for when transitions will occur.
     * <p>
     * For any given {@code ZoneRules}, this list contains the transition rules for years beyond
     * those years that have been fully defined. These rules typically refer to future daylight
     * saving time rule changes.
     * <p>
     * If the zone defines daylight savings into the future, then the list will normally be of size
     * two and hold information about entering and exiting daylight savings. If the zone does not
     * have daylight savings, or information about future changes is uncertain, then the list will
     * be empty.
     * <p>
    ...
    public boolean equals(Object otherRules) {
        ...
        if (otherRules instanceof ZoneRules) {
            ...
            return Arrays.equals(standardTransitions, other.standardTransitions) && Arrays
                    .equals(standardOffsets, other.standardOffsets) && Arrays
                            .equals(savingsInstantTransitions, other.savingsInstantTransitions) && Arrays
                                    .equals(wallOffsets, other.wallOffsets) && Arrays.equals(lastRules, other.lastRules);
        }
        return false;
    }
