 ...
 * <ul>
 * <li>Fixed offsets - a fully resolved offset from UTC/Greenwich, that uses the same offset for all
 * local date-times
 * <li>Geographical regions - an area where a specific set of rules for finding the offset from
 * UTC/Greenwich apply
 * </ul>
 * Most fixed offsets are represented by {@link ZoneOffset}. Calling {@link #normalized()} on any
 * {@code ZoneId} will ensure that a fixed offset ID will be represented as a {@code ZoneOffset}.
 * <p>
 ...
 * be normalized during creation. These IDs can be normalized to a {@code ZoneOffset} using
 * {@code normalized()}.
 * <p>
 ...
 * and {@code normalized}. However, any call to {@code getRules} will fail with
 * {@code ZoneRulesException}. This approach is designed to allow a {@link ZonedDateTime} to be
 ...
 * synchronization) on instances of {@code ZoneId} may have unpredictable results and should be
 * avoided. The {@code equals} method should be used for comparisons.
 *
 * @implSpec This abstract class has two implementations, both of which are immutable and
 *           thread-safe. One implementation models region-based IDs, the other is
 *           {@code ZoneOffset} modelling offset-based IDs. This difference is visible in
 *           serialization.
 * @since 1.8
 */
...
abstract class ZoneId implements Serializable {
     ...
     * <p>
     * This map contains a mapping of the IDs that is in line with TZDB 2005r and later, where
     * 'EST', 'MST' and 'HST' map to IDs which do not include daylight savings.
     * <p>
    ...
    public static ZoneId systemDefault() {
        return (ZoneId) (Object) TimeZone.getDefault().toZoneId();
    }

    /**
     * Gets the set of available zone IDs.
     * <p>
     * This set includes the string form of all available region-based IDs. Offset-based zone IDs
     * are not included in the returned set. The ID can be passed to {@link #of(String)} to create a
     * {@code ZoneId}.
     * <p>
     * The set of zone IDs can increase over time, although in a typical application the set of IDs
     * is fixed. Each call to this method is thread-safe.
     *
     ...
     * for use.
     * <p>
     * This method parses the ID producing a {@code ZoneId} or {@code ZoneOffset}. A
     * {@code ZoneOffset} is returned if the ID is 'Z', or starts with '+' or '-'. The result will
     * always be a valid ID for which {@link ZoneRules} can be obtained.
     * <p>
     * Parsing matches the zone ID step by step as follows.
     * <ul>
     * <li>If the zone ID equals 'Z', the result is {@code ZoneOffset.UTC}.
     * <li>If the zone ID consists of a single letter, the zone ID is invalid and
     * {@code DateTimeException} is thrown.
     * <li>If the zone ID starts with '+' or '-', the ID is parsed as a {@code ZoneOffset} using
     * {@link ZoneOffset#of(String)}.
     * <li>If the zone ID equals 'GMT', 'UTC' or 'UT' then the result is a {@code ZoneId} with the
     * same ID and rules equivalent to {@code ZoneOffset.UTC}.
     * <li>If the zone ID starts with 'UTC+', 'UTC-', 'GMT+', 'GMT-', 'UT+' or 'UT-' then the ID is
     * a prefixed offset-based ID. The ID is split in two, with a two or three letter prefix and a
     * suffix starting with the sign. The suffix is parsed as a {@link ZoneOffset#of(String)
     * ZoneOffset}. The result will be a {@code ZoneId} with the specified UTC/GMT/UT prefix and the
     * normalized offset ID as per {@link ZoneOffset#getId()}. The rules of the returned
     * {@code ZoneId} will be equivalent to the parsed {@code ZoneOffset}.
     ...
     * the form '{area}/{city}', such as 'Europe/Paris' or 'America/New_York'. This is compatible
     * with most IDs from {@link java.util.TimeZone}.
     * </ul>
     *
     * @param zoneId the time-zone ID, not null
     * @return the zone ID, not null
     ...
     * <p>
     * If the prefix is "GMT", "UTC", or "UT" a {@code ZoneId} with the prefix and the non-zero
     * offset is returned. If the prefix is empty {@code ""} the {@code ZoneOffset} is returned.
     *
    ...
    /**
     * Parses the ID, taking a flag to indicate whether {@code ZoneRulesException} should be thrown
     * or not, used in deserialization.
     *
     * @param zoneId the time-zone ID, not null
     ...
     * <p>
     * This obtains a zone based on the specified temporal. A {@code TemporalAccessor} represents an
     ...
     * it to be used as a query via method reference, {@code ZoneId::from}.
     *
    ...
    public static ZoneId from(TemporalAccessor temporal) {
        ...
        if (obj == null) {
            throw new DateTimeException("Unable to obtain ZoneId from TemporalAccessor: " + temporal + " of type " + temporal.getClass()
                    .getName());
        }
        return obj;
    }
     ...
     * <p>
     * A {@code ZoneId} can be fully represented as a {@code TemporalAccessor}. However, the
     * interface is not implemented by this class as most of the methods on the interface have no
     * meaning to {@code ZoneId}.
     * <p>
     * The returned temporal has no supported fields, with the query method supporting the return of
     * the zone using {@link TemporalQueries#zoneId()}.
     *
     ...
     * have {@code ZoneRules} equivalent to those returned by this object, however the ID returned
     * by {@code getId()} may be different.
     * <p>
     ...
     *             {@link #of(String)}, but without any exception in the case where the ID has a
     *             valid format, but is not in the known set of region-based IDs.
