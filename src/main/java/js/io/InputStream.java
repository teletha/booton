 ...
 */
@JavaAPIProvider(java.io.InputStream.class)
abstract class InputStream implements Closeable {
     ...
     * <code>b</code>. The number of bytes actually read is returned as an integer. This method
     * blocks until input data is available, end of file is detected, or an exception is thrown.
     * <p>
     * If the length of <code>b</code> is zero, then no bytes are read and <code>0</code> is
     * returned; otherwise, there is an attempt to read at least one byte. If no byte is available
     * because the stream is at the end of the file, the value <code>-1</code> is returned;
     ...
     * <code>b</code>. Let <i>k</i> be the number of bytes actually read; these bytes will be stored
     * in elements <code>b[0]</code> through <code>b[</code><i>k</i><code>-1]</code>, leaving
     * elements <code>b[</code><i>k</i><code>]</code> through <code>b[b.length-1]</code> unaffected.
     * <p>
     ...
     * @exception IOException If the first byte cannot be read for any reason other than the end of
     *                the file, if the input stream has been closed, or if some other I/O error
     *                occurs.
     ...
     * <code>len</code>. Let <i>k</i> be the number of bytes actually read; these bytes will be
     * stored in elements <code>b[off]</code> through <code>b[off+</code><i>k</i><code>-1]</code>,
     * leaving elements <code>b[off+</code><i>k</i><code>]</code> through <code>b[off+len-1]</code>
     * unaffected.
     * <p>
     ...
     * such call results in an <code>IOException</code>, that exception is returned from the call to
     * the <code>read(b,</code> <code>off,</code> <code>len)</code> method. If any subsequent call
     * to <code>read()</code> results in a <code>IOException</code>, the exception is caught and
     * treated as if it were end of file; the bytes read up to that point are stored into
     * <code>b</code> and the number of bytes read before the exception occurred is returned. The
     * default implementation of this method blocks until the requested amount of input data
     * <code>len</code> has been read, end of file is detected, or an exception is thrown.
     ...
     *                file, or if the input stream has been closed, or if some other I/O error
     *                occurs.
     * @exception NullPointerException If <code>b</code> is <code>null</code>.
     ...
     * <p>
     * When this stream reaches end of stream, further invocations of this method will return an
     * empty byte array.
     * <p>
     * Note that this method is intended for simple cases where it is convenient to read all bytes
     * into a byte array. It is not intended for reading input streams with large amounts of data.
     * <p>
     * The behavior for the case where the input stream is <i>asynchronously closed</i>, or the
     * thread interrupted during the read, is highly input stream specific, and therefore not
     * specified.
     * <p>
     * If an I/O error occurs reading from the input stream, then it may do so after some, but not
     * all, bytes have been read. Consequently the input stream may not be at end of stream and may
     * be in an inconsistent state. It is strongly recommended that the stream be promptly closed if
     * an I/O error occurs.
     *
     ...
     * <p>
     * The length of the returned array equals the number of bytes read from the stream. If
     ...
     * empty byte array.
     * <p>
     * Note that this method is intended for simple cases where it is convenient to read the
     * specified number of bytes into a byte array. The total amount of memory allocated by this
     * method is proportional to the number of bytes read from the stream which is bounded by
     * {@code len}. Therefore, the method may be safely called with very large values of {@code len}
    ...
    /**
     * Reads the requested number of bytes from the input stream into the given byte array. This
     * method blocks until {@code len} bytes of input data have been read, end of stream is
     * detected, or an exception is thrown. The number of bytes actually read, possibly zero, is
     * returned. This method does not close the input stream.
     * <p>
     * In the case where end of stream is reached before {@code len} bytes have been read, then the
     * actual number of bytes read will be returned. When this stream reaches end of stream, further
     * invocations of this method will return zero.
     * <p>
     * If {@code len} is zero, then no bytes are read and {@code 0} is returned; otherwise, there is
     * an attempt to read up to {@code len} bytes.
     * <p>
     * The first byte read is stored into element {@code b[off]}, the next one in to
     * {@code b[off+1]}, and so on. The number of bytes read is, at most, equal to {@code len}. Let
     * <i>k</i> be the number of bytes actually read; these bytes will be stored in elements
     * {@code b[off]} through {@code b[off+}<i>k</i>{@code -1]}, leaving elements
     ...
     * <p>
     * If an I/O error occurs reading from the input stream, then it may do so after some, but not
     * all, bytes of {@code b} have been updated with data from the input stream. Consequently the
     * input stream and {@code b} may be in an inconsistent state. It is strongly recommended that
     * the stream be promptly closed if an I/O error occurs.
     *
     ...
     * number of bytes, possibly <code>0</code>. This may result from any of a number of conditions;
     * reaching end of file before <code>n</code> bytes have been skipped is only one possibility.
     * The actual number of bytes skipped is returned. If {@code n} is negative, the {@code skip}
     * method for class {@code InputStream} always returns 0, and no bytes are skipped. Subclasses
     * may handle the negative value differently.
     * <p>
     * The <code>skip</code> method of this class creates a byte array and then repeatedly reads
     ...
     * @exception IOException if the stream does not support seek, or if some other I/O error
     *                occurs.
     */
     ...
     * <code>mark</code> and stands ready to supply those same bytes again if and whenever the
     * method <code>reset</code> is called. However, the stream is not required to remember any data
     * at all if more than <code>readlimit</code> bytes are read from the stream before
     ...
     * number of bytes read from the stream since <code>mark</code> was last called is larger than
     * the argument to <code>mark</code> at that last call, then an <code>IOException</code> might
     * be thrown.
     ...
     * the next input data as of the time of the call to <code>reset</code>.
     * </ul>
     ...
     * Whether or not <code>mark</code> and <code>reset</code> are supported is an invariant
     * property of a particular input stream instance. The <code>markSupported</code> method of
