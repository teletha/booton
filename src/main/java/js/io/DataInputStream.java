...
class DataInputStream extends FilterInputStream implements DataInput {
    ...
    /**
     * Reads some number of bytes from the contained input stream and stores them into the buffer
     * array <code>b</code>. The number of bytes actually read is returned as an integer. This
     * method blocks until input data is available, end of file is detected, or an exception is
     * thrown.
     * <p>
     * If <code>b</code> is null, a <code>NullPointerException</code> is thrown. If the length of
     * <code>b</code> is zero, then no bytes are read and <code>0</code> is returned; otherwise,
     * there is an attempt to read at least one byte. If no byte is available because the stream is
     * at end of file, the value <code>-1</code> is returned; otherwise, at least one byte is read
     ...
     * <p>
     * The <code>read(b)</code> method has the same effect as: <blockquote><pre>
     * read(b, 0, b.length)
     * </pre></blockquote>
     *
     ...
     * bytes. An attempt is made to read as many as <code>len</code> bytes, but a smaller number may
     ...
     *
     * @param b the buffer into which the data is read.
     * @exception EOFException if this input stream reaches the end before reading all the bytes.
     ...
     *                been closed and the contained input stream does not support reading after
     ...
     * @exception EOFException if this input stream reaches the end before reading four bytes.
     * @exception IOException the stream has been closed and the contained input stream does not
     *                support reading after close, or another I/O error occurs.
     * @see java.io.FilterInputStream#in
     */
    ...
    /**
     * See the general contract of the <code>readLine</code> method of <code>DataInput</code>.
     * <p>
     * Bytes for this operation are read from the contained input stream.
     *
     * @deprecated This method does not properly convert bytes to characters. As of JDK&nbsp;1.1,
     *             the preferred way to read lines of text is via the
     *             <code>BufferedReader.readLine()</code> method. Programs that use the
     *             <code>DataInputStream</code> class to read lines can be converted to use the
     *             <code>BufferedReader</code> class by replacing code of the form: <blockquote>
     *             <pre>
     *     DataInputStream d =&nbsp;new&nbsp;DataInputStream(in);
     * </pre></blockquote> with: <blockquote><pre>
     *     BufferedReader d
     ...
     * @exception IOException the stream has been closed and the contained input stream does not
     *                support reading after close, or another I/O error occurs.
     * @exception UTFDataFormatException if the bytes do not represent a valid modified UTF-8
    ...
    /**
     * Reads from the stream <code>in</code> a representation of a Unicode character string encoded
     * in <a href="DataInput.html#modified-utf-8">modified UTF-8</a> format; this string of
     * characters is then returned as a <code>String</code>. The details of the modified UTF-8
     * representation are exactly the same as for the <code>readUTF</code> method of
     * <code>DataInput</code>.
     *
