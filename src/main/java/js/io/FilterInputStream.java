...
/**
 * @version 2015/08/08 16:58:11
 */
@JavaAPIProvider(java.io.FilterInputStream.class)
class FilterInputStream extends InputStream {
    ...
    /**
     * Reads up to <code>byte.length</code> bytes of data from this input stream into an array of
     ...
     * result. It is important that it does <i>not</i> do <code>in.read(b)</code> instead; certain
     * subclasses of <code>FilterInputStream</code> depend on the implementation strategy actually
     * used.
     *
    ...
    /**
     * Skips over and discards <code>n</code> bytes of data from the input stream. The
     * <code>skip</code> method may, for a variety of reasons, end up skipping over some smaller
     * number of bytes, possibly <code>0</code>. The actual number of bytes skipped is returned.
     * <p>
     ...
     * the same bytes.
     * <p>
     * The <code>readlimit</code> argument tells this input stream to allow that many bytes to be
     ...
     * <p>
     * Stream marks are intended to be used in situations where you need to read ahead a little to
     * see what's in the stream. Often this is most easily done by invoking some general parser. If
     * the stream is of the type handled by the parse, it just chugs along happily. If the stream is
     * not of that type, the parser should toss an exception when it fails. If this happens within
     ...
     * This method simply performs <code>in.markSupported()</code>.
     *
     * @return <code>true</code> if this stream type supports the <code>mark</code> and
     *         <code>reset</code> method; <code>false</code> otherwise.
     * @see java.io.FilterInputStream#in
