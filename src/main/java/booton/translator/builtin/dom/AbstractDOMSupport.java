...
/**
 * @version 2012/12/02 10:04:09
 */
abstract class AbstractDOMSupport<T> extends Translator<T> {
     ...
     *                allow children of the type of the <code>newChild</code> node, or if the node
     *                to insert is one of this node's ancestors or this node itself, or if this node
     *                is of type <code>Document</code> and the DOM application attempts to insert a
     *                second <code>DocumentType</code> or <code>Element</code> node. <br>
     *                WRONG_DOCUMENT_ERR: Raised if <code>newChild</code> was created from a
     ...
     * @exception DOMException HIERARCHY_REQUEST_ERR: Raised if this node is of a type that does not
     *                allow children of the type of the <code>newChild</code> node, or if the node
     *                to put in is one of this node's ancestors or this node itself, or if this node
     *                is of type <code>Document</code> and the result of the replacement operation
     *                would add a second <code>DocumentType</code> or <code>Element</code> on the
     ...
     * @exception DOMException HIERARCHY_REQUEST_ERR: Raised if this node is of a type that does not
     *                allow children of the type of the <code>newChild</code> node, or if the node
     *                to append is one of this node's ancestors or this node itself, or if this node
     *                is of type <code>Document</code> and the DOM application attempts to append a
     *                second <code>DocumentType</code> or <code>Element</code> node. <br>
     *                WRONG_DOCUMENT_ERR: Raised if <code>newChild</code> was created from a
     *                different document than the one that created this node. <br>
     *                NO_MODIFICATION_ALLOWED_ERR: Raised if this node is readonly or if the
     *                previous parent of the node being inserted is readonly. <br>
     *                NOT_SUPPORTED_ERR: if the <code>newChild</code> node is a child of the
     *                <code>Document</code> node, this exception might be raised if the DOM
     *                implementation doesn't support the removal of the <code>DocumentType</code>
     *                child or <code>Element</code> child.
     * @since DOM Level 3
     */
     ...
     * data. User data associated to the imported node is not carried over. However, if any
     * <code>UserDataHandlers</code> has been specified along with the associated data these
     * handlers will be called with the appropriate parameters before this method returns. <br>
     ...
     * Cloning an <code>Attr</code> directly, as opposed to be cloned as part of an
     * <code>Element</code> cloning operation, returns a specified attribute (<code>specified</code>
     * is <code>true</code>). Cloning an <code>Attr</code> always clones its children, since they
     * represent its value, no matter whether this is a deep clone or not. Cloning an
     * <code>EntityReference</code> automatically constructs its subtree if a corresponding
     * <code>Entity</code> is available, no matter whether this is a deep clone or not. Cloning any
     ...
     * <code>EntityReference</code> clone are readonly . In addition, clones of unspecified
     * <code>Attr</code> nodes are specified. And, cloning <code>Document</code>,
     ...
     * lookups) that depend on a particular document tree structure are to be used. If the parameter
     * "normalize-characters" of the <code>DOMConfiguration</code> object attached to the
     * <code>Node.ownerDocument</code> is <code>true</code>, this method will also fully normalize
     * the characters of the <code>Text</code> nodes.
     * <p >
     * <b>Note:</b> In cases where the document contains <code>CDATASections</code>, the normalize
     * operation alone may not be sufficient, since XPointers do not differentiate between
     ...
     * attribute, which holds the qualified name, as well as the <code>tagName</code> and
     * <code>name</code> attributes of the <code>Element</code> and <code>Attr</code> interfaces,
     * when applicable. <br>
     ...
     * </tr>
     * <tr>
     * <td valign='top' rowspan='1' colspan='1'>ELEMENT_NODE, ATTRIBUTE_NODE, ENTITY_NODE,
     * ENTITY_REFERENCE_NODE, DOCUMENT_FRAGMENT_NODE</td>
     * <td valign='top' rowspan='1' colspan='1'>concatenation of the <code>textContent</code>
     * attribute value of every child node, excluding COMMENT_NODE and PROCESSING_INSTRUCTION_NODE
     ...
     * attribute for <code>Attr</code> nodes, the <code>schemaTypeInfo</code> attribute for
     * <code>Attr</code> and <code>Element</code> nodes, the
     * <code>Text.isElementContentWhitespace</code> attribute for <code>Text</code> nodes, as well
     * as any user data or event listeners registered on the nodes.
     * <p >
     * <b>Note:</b> As a general rule, anything not mentioned in the description above is not
     ...
     *            the name of the feature will be ignored since it is not significant in the context
     *            of this method.
     * @param version This is the version number of the feature to test.
     * @return Returns an object which implements the specialized APIs of the specified feature and
     *         version, if any, or <code>null</code> if there is no object which implements
