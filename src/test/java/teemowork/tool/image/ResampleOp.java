...
public class ResampleOp extends AdvancedResizeOp {
    ...
    static class SubSamplingData {
        ...
        private final int numContributors; // the primary index length for the 2Dim arrays :
    ...
    public BufferedImage doFilter(BufferedImage srcImg, BufferedImage dest, int dstWidth, int dstHeight) {
        ...
        if (srcImg.getType() == BufferedImage.TYPE_BYTE_BINARY || srcImg.getType() == BufferedImage.TYPE_BYTE_INDEXED || srcImg.getType() == BufferedImage.TYPE_CUSTOM)
            srcImg = ImageUtils.convert(srcImg, srcImg.getColorModel().hasAlpha() ? BufferedImage.TYPE_4BYTE_ABGR
    ...
    static SubSamplingData createSubSampling(ResampleFilter filter, int srcSize, int dstSize) {
        ...
        if (scale < 1.0f) {
            ...
            numContributors = (int) (width * 2.0f + 2); // Heinz: added 1 to be save with the
                                                        // ceilling
    ...
    private void horizontallyFromSrcToWork(BufferedImage srcImg, byte[][] workPixels, int start, int delta) {
        ...
        final int[] tempPixels = new int[srcWidth]; // Used if we work on int based bitmaps, later
                                                    // used to keep channel values
        final byte[] srcPixels = new byte[srcWidth * nrChannels]; // create reusable row to minimize
                                                                  // memory overhead
    ...
    private void horizontallyFromSrcToWorkGray(BufferedImage srcImg, byte[][] workPixels, int start, int delta) {
        final int[] tempPixels = new int[srcWidth]; // Used if we work on int based bitmaps, later
                                                    // used to keep channel values
        final byte[] srcPixels = new byte[srcWidth]; // create reusable row to minimize memory
                                                     // overhead
    ...
    protected int getResultBufferedImageType(BufferedImage srcImg) {
        return nrChannels == 3 ? BufferedImage.TYPE_3BYTE_BGR : (nrChannels == 4 ? BufferedImage.TYPE_4BYTE_ABGR
                : (srcImg.getSampleModel().getDataType() == DataBuffer.TYPE_USHORT ? BufferedImage.TYPE_USHORT_GRAY
