...
/**
 * A {@link NavigableSet} implementation based on a {@link TreeMap2}. The elements are ordered using
 * their {@linkplain Comparable natural ordering}, or by a {@link Comparator} provided at set
 * creation time, depending on which constructor is used.
 * <p>
 * This implementation provides guaranteed log(n) time cost for the basic operations ({@code add},
 ...
 * must be <i>consistent with equals</i> if it is to correctly implement the {@code Set} interface.
 * (See {@code Comparable} or {@code Comparator} for a precise definition of <i>consistent with
 * equals</i>.) This is so because the {@code Set} interface is defined in terms of the
 * {@code equals} operation, but a {@code TreeSet} instance performs all element comparisons using
 * its {@code compareTo} (or {@code compare}) method, so two elements that are deemed equal by this
 * method are, from the standpoint of the set, equal. The behavior of a set <i>is</i> well-defined
 * even if its ordering is inconsistent with equals; it just fails to obey the general contract of
 * the {@code Set} interface.
 * <p>
 * <strong>Note that this implementation is not synchronized.</strong> If multiple threads access a
 * tree set concurrently, and at least one of the threads modifies the set, it <i>must</i> be
 ...
 * {@link Collections#synchronizedSortedSet Collections.synchronizedSortedSet} method. This is best
 * done at creation time, to prevent accidental unsynchronized access to the set:
 * 
 * <pre>
 *   SortedSet s = Collections.synchronizedSortedSet(new TreeSet(...));</pre>
 * <p>
 * The iterators returned by this class's {@code iterator} method are <i>fail-fast</i>: if the set
 * is modified at any time after the iterator is created, in any way except through the iterator's
 * own {@code remove} method, the iterator will throw a {@link ConcurrentModificationException}.
...
public class TreeSet2<E> extends AbstractSet<E> implements NavigableSet<E>, Cloneable, java.io.Serializable {
     ...
     * set. If the user attempts to add an element to the set that violates this constraint (for
     * example, the user attempts to add a string element to a set whose elements are integers), the
    ...
    /**
     * Constructs a new, empty tree set, sorted according to the specified comparator. All elements
     * inserted into the set must be <i>mutually comparable</i> by the specified comparator:
     * {@code comparator.compare(e1,
     * e2)} must not throw a {@code ClassCastException} for any elements {@code e1} and {@code e2}
     * in the set. If the user attempts to add an element to the set that violates this constraint,
    ...
    public TreeSet2(Comparator<? super E> comparator) {
        this(new TreeMap2<>(comparator));
    }

    /**
     * Constructs a new tree set containing the elements in the specified collection, sorted
     * according to the <i>natural ordering</i> of its elements. All elements inserted into the set
     * must implement the {@link Comparable} interface. Furthermore, all such elements must be
     ...
     * contains the element, the call leaves the set unchanged and returns {@code false}.
     * 
     ...
     * element {@code e} such that <tt>(o==null&nbsp;?&nbsp;e==null&nbsp;:&nbsp;o.equals(e))</tt>,
     * if this set contains such an element. Returns {@code true} if this set contained the element
     ...
     *             and this set uses natural ordering, or its comparator does not permit null
     *             elements
     */
    public boolean addAll(Collection<? extends E> c) {
        // Use linear-time version if applicable
        if (m.size() == 0 && c.size() > 0 && c instanceof SortedSet && m instanceof TreeMap2) {
    ...
    public NavigableSet<E> subSet(E fromElement, boolean fromInclusive, E toElement, boolean toInclusive) {
        return new TreeSet2<>(m.subMap(fromElement, fromInclusive, toElement, toInclusive));
    }
